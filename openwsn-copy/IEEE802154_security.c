...
//不使用L2SEC时，还将调用以下7个功能。 这有助于在不使用L2SEC的情况下自动测试SECJOIN，
//并确保节点开始发送EB的顺序。 在当前实现中，仅在节点收到来自JRC的加入响应后才发送EB。
void IEEE802154_security_init(void) {

   // By default, we assume that no dynamic keying (SEC JOIN) is used ,通常情况下，任务没有可用的动态秘钥
