...
//设置回调函数(callbqack = Cb)
    ...
    // have the radio start its timer and assign ieee802154e timer with highest priority 设置时钟，周期为slot时长触发
...
/**
\brief Indicates a new slot has just started.  标识一个新slot开启

This function executes in ISR mode, when the new slot timer fires.
*/
void isr_ieee154e_newSlot(opentimers_id_t id) {

    ieee154e_vars.startOfSlotReference = opentimers_getCurrentCompareValue();//获取opentimer2的currentCompareValue变量。
    opentimers_scheduleAbsolute(//此功能将调度一个计时器，该计时器在计时器计数值达到duartio+referece 时到期
        ieee154e_vars.timerId,                  // timerId
    ...
    if (ieee154e_vars.isSync==FALSE) {//若未同步
        if (idmanager_getIsDAGroot()==TRUE) {
            changeIsSync(TRUE);
            ieee154e_resetAsn();
